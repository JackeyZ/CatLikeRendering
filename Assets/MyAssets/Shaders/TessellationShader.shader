Shader "Custom/Tessellation" // 曲面细分
{
    Properties
    {
        ...
        _ParallaxStrength("Parallax Strength", Range(0, 1)) = 0                 // 视差强度（细分着色器中变成顶点偏移强度）
        ...
        //_WireframeColor ("Wireframe Color", Color) = (0, 0, 0)                  // 线框颜色
        //_WireframeSmoothing ("Wireframe Smoothing", Range(0, 10)) = 1           // 线框过渡宽度（像素）
        //_WireframeThickness ("Wireframe Thickness", Range(0, 10)) = 1           // 线框宽度（像素）
        
        _TessellationUniform ("Tessellation Uniform", Range(1, 64)) = 1                // 细分程度（硬件只支持64及以下的细分）
        _TessellationEdgeLength ("_Tessellation Edge Length", Range(5, 100)) = 50      // 三角面的边，多长距离细分一次（裁剪空间下的距离）
    }
    ...
    CGINCLUDE
        ...
        // 表明启用视差顶点偏移(顶点偏移里采样的视差贴图需要连续没有接缝，贴图出现不连续的时候会造成表面会撕裂)
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            //#pragma multi_compile_instancing  // 细分着色器不支持实例化，这里屏蔽
            ...
            //#include "CustomFlatWireframe.cginc"
